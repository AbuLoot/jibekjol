  ...
  <div class="container">
    ...
    @if (session('result'))
      <div class="alert alert-info">
        ...
        <h4>Was on the border tracks count: {{ session('result')['onTheBorderTracksCount'] }}pcs</h4>
      ...
      <div class="track-item mb-2">
        ...
        <div class="row">
          ...
          <div class="col-2 col-lg-2 text-end">
            @if($track->status != $statusOnTheBorder->id)
              <div class="d-grid">
                <button  wire:click="btnToMark('{{ $track->code }}')" type="button" wire:loading.attr="disabled" class="btn btn-primary"><i class="bi bi-globe2"></i> <span class="d-none d-sm-inline">To mark</span></button>
              </div>
            @endif
          </div>
        </div>
    ...
    <div class="row">
      <div class="col-12 col-sm-4 mb-2">
        <form wire:submit.prevent="toMark">
          ...
          <div class="input-group">
            ...
            <button type="submit" id="toMark" wire:loading.attr="disabled" class="btn btn-primary btn-lg"><i class="bi bi-globe2"></i> To mark</button>
          </div>
      ...
      <div class="col-12 col-sm-8">
        @if($mode == 'group')
          ...
          <?php
            ...
            // Grouped by date
            $todayGroup         = $allSentTracks->where('updated_at', '>', $today.' 00:00:00')->where('updated_at', '<=', $today.' 23:59:59');
            $yesterdayGroup     = $allSentTracks->where('updated_at', '>', $yesterday.' 00:00:00')->where('updated_at', '<', $yesterday.' 23:59:59');
            $twoDaysAgoGroup    = $allSentTracks->where('updated_at', '>', $twoDaysAgo.' 00:00:00')->where('updated_at', '<', $twoDaysAgo.' 23:59:59');
            $threeDaysAgoGroup  = $allSentTracks->where('updated_at', '>', $threeDaysAgo.' 00:00:00')->where('updated_at', '<', $threeDaysAgo.' 23:59:59');
            $fourDaysAgoGroup   = $allSentTracks->where('updated_at', '>', $fourDaysAgo.' 00:00:00')->where('updated_at', '<', $fourDaysAgo.' 23:59:59');
            $fiveDaysAgoGroup   = $allSentTracks->where('updated_at', '>', $fiveDaysAgo.' 00:00:00')->where('updated_at', '<', $fiveDaysAgo.' 23:59:59');
            $sixDaysAgoGroup    = $allSentTracks->where('updated_at', '>', $sixDaysAgo.' 00:00:00')->where('updated_at', '<', $sixDaysAgo.' 23:59:59');
            $previousWeekGroup  = $allSentTracks->where('updated_at', '>', $previousWeek.' 00:00:00')->where('updated_at', '<', $previousWeek.' 23:59:59');
            $twoWeekAgoGroup    = $allSentTracks->where('updated_at', '>', $twoWeekAgo.' 00:00:00')->where('updated_at', '<', $twoWeekAgo.' 23:59:59');
            $prevTimeGroup      = $allSentTracks->where('updated_at', '<', $twoWeekAgo.' 00:00:00');
                ...
                <div class="border bg-sent rounded p-2">
                  <div class="row">
                    ...
                    <div class="col-12s col-md-5 text-end">
                      ...
                      <button type="button" wire:click="markGroupByDate('{{ $group['dateFrom'] }}', '{{ $group['dateTo'] }}')" wire:loading.attr="disabled" onclick="return confirm('Сonfirm action?') || event.stopImmediatePropagation()" class="btn btn-success btn-lg"><i class="bi bi-globe2"></i> Mark group</button>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <form action="/{{ $lang }}/admin/upload-tracks" method="post" enctype="multipart/form-data">
          @csrf
          <input type="hidden" name="storageStage" value="on-the-border">
          <div class="modal-header">
