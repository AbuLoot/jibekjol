      ...
      <div class="modal-content">
        <form wire:submit.prevent="saveTrack">
          <div class="modal-header">
            <h1 class="modal-title fs-5" id="modalLabel">{{ __('app.edit_track') }}</h1>
            <button type="button" id="closeEditTrack" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <div class="mb-3">
              <label for="code" class="form-label">{{ __('app.track_code') }}</label>              
          ...
          <div class="modal-footer">
            <button type="submit" class="btn btn-primary btn-lg">{{ __('app.save') }}</button>
          </div>
        </form>
      </div>
    </div>
